...
### 2.3 运行环境
- **前端客户端**：全端响应式 Web，以及未来可扩展的 App 内嵌 H5。
- **后端服务端**：基于 K8s (Kubernetes) 编排的云原生 Docker 微服务集群组。
- **部署理念**：利用 CDN 就近分发静态资产，API 接入层通过多级 Load Balancer (如 Nginx -> API Gateway) 进行流量分发。
...
#### ACTIVITY-02：高并发检索（C端搜索浏览）
- **描述**：用户可以结合时间、地域、明星IP标签进行筛选。读请求极高。
- **约束**：详情页信息缓存化（CDN/Redis），减缓 DB QPS（Query Per Second）。
- **优先级**：高

---

### 3.3 极限并发抢票引擎（ENROLL/BUY）

#### ENROLL-01：分布式防超卖原子扣减
- **描述**：百万用户在同一秒钟发起的开票抢购。
- **后端主流程**：
  1. **网关拦截/风控**：识别恶意机器抓取（本期忽略黑产判定，仅做频控逻辑模拟）。
  2. **Cache 层绝对原子库存扣减**：通过 Redis Lua 校验 `stock > 0` 并原子 -1，否则 `return 0`。杜绝线程死锁与数据库锁表。
  3. **基于 MQ 的排队承诺**：返回前端 `HTTP 202 Accepted`，并在 Redis 设置用户进入"排队排号状态"。
- **优先级**：最高级别核心
